...
export default function PostPreviewCard({
  ...
  const sizes = type === 'buy' ? 'w-48 h-36 md:w-96 md:h-72' :
    cols === 1 ? 'w-96 h-72' : 'w-48 sm:w-72 md:w-96 h-36 md:h-72';
      ...
      <div className="flex flex-col overflow-hidden shadow line-break max-w-lg">
        <div className='flex gap-3 font-medium items-center p-1 md:p-2'>
          <div className="flex-none">
            <UserProfile user={post.user} displayAvatar displayUsername={false} displayDM />
          </div>
          <div className='flex-1 pr-1 flex flex-row justify-between'>
            <div className='flex flex-col'>
              <div className='flex flex-row gap-2 text-sm font-normal items-center'>
                <UserProfile user={post.user} displayAvatar={false} displayUsername displayDM />
            ...
            <div className='relative'>
              <button type='submit' onClick={() => handleOpen()} className='group'>
                ...
                <div className='absolute top-1/2 left-1/2 z-50 -translate-x-1/2 -translate-y-1/2'>
                  <ChatBubbleOvalLeftIcon color='white' fill='white' className='opacity-0 group-hover:opacity-100 w-8 h-8 md:w-9 md:h-9' />
                </div>
              </button>
              <div className='absolute top-1/2 left-1/2 z-50 -translate-x-1/2 -translate-y-1/2'>
    ...
    <div className="flex flex-col overflow-hidden shadow line-break">
      <div className='flex p-1 md:p-2 font-medium'>
        <div className='flex flex-row gap-2 items-center'>
          <UserProfile user={post.user} displayAvatar displayUsername displayDM />
