...
export default function Posts() {
  const categoryId = useAppSelector(
    (state: RootState) => state.categoriesSlice.categoryId,
  )
  ...
  if (status === 'loading') {
    if (type === 'buy')
      return (
        <div className='flex justify-center'>
          <div
            className={`grid gap-2 max-w-lg md:gap-2 lg:gap-2 grid-cols-1 items-start`}
          >
            <MockCards />
          </div>
          <div ref={ref} />
    ...
    return (
      <div
        className={`grid gap-4 md:gap-8 lg:gap-12 grid-cols-2 md:grid-cols-2 lg:grid-cols-3 items-start`}
      >
        {<MockCards />}
      </div>
    )
